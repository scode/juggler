...
mod tests {
    ...
    fn load_todos_parses_comments() {
        let todos = super::load_todos("TODOs.yaml").expect("load TODOs");
...
"#;
        ...
        let item: super::TodoItem = serde_yaml::from_str(yaml).expect("deserialize from YAML");
    ...
    fn store_todos_roundtrip() {
        ...
        // Store the todos
        super::store_todos(&test_todos, &test_file).expect("store todos");

        // Verify the file was created
        assert!(test_file.exists());

        // Load them back
        let loaded_todos = super::load_todos(&test_file).expect("load todos");
    ...
    fn load_todos_handles_missing_file() {
        ...
        let todos =
            super::load_todos(&non_existent_file).expect("load todos from non-existent file");
